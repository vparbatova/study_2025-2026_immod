...
df_lv[!, :dpredator_dt] = p_lv[3] .* df_lv.prey .* df_lv.predator .-
...
color=:red, linestyle=:dash, linewidth=1.5, label="Изоклина
хищников (dy/dt=0)")
...
color=:green, linestyle=:dash, linewidth=1.5, label="Изоклина
жертв (dx/dt=0)")
...
spectrum = max.(spectrum, 1e-16)
return freq, spectrum
end
# Вычисление периодов колебаний
freq_prey, spectrum_prey = compute_fft(df_lv.prey .-
mean(df_lv.prey), dt_lv)
freq_predator, spectrum_predator = compute_fft(df_lv.predator .-
mean(df_lv.predator), dt_lv)
...
plot!(plt6[3], df_lv.prey, df_lv.predator, label=false, color=:blue,
linewidth=1.5,
...
df_lv.t)
peak_time_predator, peak_value_predator =
find_first_peak(df_lv.predator, df_lv.t)
