...
model = StandardABM(Person, space; properties, rng,
...
infected_ids = sample(rng, city_agents, Is[city];
replace = false)
...
neighbors = [a for a in agents_in_position(agent.pos, model) if
a.id != agent.id]
...
if agent.status == :I && agent.days_infected ≥
...
infected_count(model) = count(a.status == :I for a in
allagents(model))
recovered_count(model) = count(a.status == :R for a in
allagents(model))
susceptible_count(model) = count(a.status == :S for a in
allagents(model))
