...
count(a.status == :I for a in allagents(model)) /
nagents(model)
peak_infected = 0.0
...
final_recovered = count(a.status == :R for a in
allagents(model)) / nagents(model)
...
plot!(grouped.beta, grouped.mean_deaths ./ 3000, label = "Доля
умерших", marker = :diamond)
savefig(plotsdir("beta_scan.png"))
println("Результаты сохранены в data/beta_scan_all.csv и
plots/beta_scan.png")
